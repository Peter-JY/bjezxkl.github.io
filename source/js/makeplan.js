...
function getMusic()	// 这个函数只负责获取music.xml并转换为js对象
{
	...
	{
		case "2025":
			var filePath = "https://bjezxkl.pages.dev/database/music_2025.xml"; // XML文件路径
			break;
		case "2024":
			var filePath = "https://bjezxkl.pages.dev/database/music_2024.xml"; // XML文件路径
			break;
		default:
			var filePath = "https://bjezxkl.pages.dev/database/music_2025.xml"; // XML文件路径
			break;
...
//		filePath = "https://bjezxkl.pages.dev/database/music_full.xml";
	fetch(filePath)
	.then(response => {
		if (!response.ok) {
...
function getSchedulingContribution()
{
	...
	$.ajax({
		url: "https://bjezxkl.pages.dev/admin",
		type: 'POST',
...
function fillConInfo(con_info)
{
	...
	switch (mid_type)
	{
		case "ncmid":
			...
			$('.coninfos-text span.coninfos#ncmid').html("<a class='mid' href='https://music.163.com/#/song?id=" + ncmid + "'>" + ncmid + "</a>")
			break;
		case "qqmid-id":
		case "qqmid-mid":
			$('.coninfos-text span.coninfos#qqmid').parent().css('display', '');
			$('.coninfos-text span.coninfos#songtype').parent().css('display', '');
			$('.coninfos-text span.coninfos#qqmid').html("<a class='mid' href='https://y.qq.com/n/ryqq/songDetail/" + qqmid + "?songtype=" + songtype + "'>" + qqmid + "</a>")
			$('.coninfos-text span.coninfos#songtype').html("<a class='mid' href='https://y.qq.com/n/ryqq/songDetail/" + qqmid + "?songtype=" + songtype + "'>" + songtype + "</a>")
			break;
		case "kgmid":
			$('.coninfos-text span.coninfos#kgmid').parent().css('display', '');
			$('.coninfos-text span.coninfos#kgmid').html("<a class='mid' href='https://www.kugou.com/mixsong/" + kgmid + ".html'>" + kgmid + "</a>")
			break;
		case "av":
		case "BV":
			$('.coninfos-text span.coninfos#BV').parent().css('display', '');
			$('.coninfos-text span.coninfos#BV').html("<a class='mid' href='https://www.bilibili.com/video/" + ((BV != "" && BV != undefined) ? BV : av) + "/'>" + BV + "</a>")
			break;
		case "ytmid":
			$('.coninfos-text span.coninfos#ytmid').parent().css('display', '');
			$('.coninfos-text span.coninfos#ytmid').html("<a class='mid' href='https://www.youtube.com/watch?v=" + ytmid + "'>" + ytmid + "</a>")
			break;
		case "ncrid":
			$('.coninfos-text span.coninfos#ncrid').parent().css('display', '');
			$('.coninfos-text span.coninfos#ncrid').html("<a class='mid' href='https://music.163.com/#/program?id=" + ncrid + "'>" + ncrid + "</a>")
			break;
		case "links":
			$('.coninfos-text span.coninfos#links').parent().css('display', '');
			$('.coninfos-text span.coninfos#links').html("<a class='mid' href='" + links + "'>" + links + "</a>")
			break;
	}
...
async function getMusicLink(con_info, mid_type) { return new Promise((resolve, reject) =>
{
	...
	switch (mid_type)
	{
		case "ncmid":
			...
			$.ajax({
				url: "https://bjezxkl.pages.dev/music_api",
				type: 'POST',
				data: JSON.stringify(postData),	// Cloudflare Functions不支持JavaScript对象，所以只能以json形式发送
				dataType: 'json',	// 返回也得是json形式
		...
		case "qqmid-id":
		case "qqmid-mid":
			var murl = "https://y.qq.com/n/ryqq/songDetail/" + con_info.qqmid + "?songtype=" + songtype
			switch (mid_type)
			{
				case "qqmid-id":
